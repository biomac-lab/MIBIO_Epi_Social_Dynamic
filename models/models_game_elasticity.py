...
# Models                                    
######################################################################### 
def sis_replicator(graph, max_time, params, local=False, gameFixedFraction=0.3, gameFixedStrategy='defect'):
    ...
    # Always D/C
    listDefactors = list(np.where(game_states[0,:] == 1)[0])
    N_fixedDefactors = int(len(listDefactors)*gameFixedFraction)

    listCooperators = list(np.where(game_states[0,:] == 1)[0])
    N_fixedCooperators = int(len(listCooperators)*gameFixedFraction)

    alwaysDefect = random.sample(listDefactors,N_fixedDefactors)
    alwaysCooperate = random.sample(listCooperators,N_fixedCooperators)

    fixedStrategy = None
    if gameFixedStrategy == 'defect':
        fixedStrategy = alwaysDefect
    else:
        fixedStrategy = alwaysCooperate
    ...
    for t in range(1, max_time):
        ...
        for idx, node_i in enumerate(G.nodes()):
            # check is node is fixed
            if node_i not in fixedStrategy:
                node_i_nn_ = [neig for neig in G.neighbors(node_i)]        # get neighbors
                node_i_nn  = np.array([id_node[nn] for nn in node_i_nn_ ])
                ...
                if gameFixedStrategy == 'defect':
                    game_states[node_i, t] = 1
                else:
                    game_states[node_i, t] = 0
...
def run_model(model, graph, params, n_iters=10, max_time=300, num_checkpoints=1,  local=False, gameFixedFraction=0.3, gameFixedStrategy='defect', path_to_save_checkpoints=None):

    df_list = []
    chk_point = 0
    for n_iter in range(n_iters):

        game_states, infection_states, payoffs = model(graph, 
                                                       max_time, 
                                                       params, 
                                                       local=local, 
                                                       gameFixedFraction=gameFixedFraction, 
                                                       gameFixedStrategy=gameFixedStrategy)
