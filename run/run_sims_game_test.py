...
for idx, r in tqdm(df_param_run.iterrows()):
    ...
    if not os.path.isdir( os.path.join(results_path, args.game_strategy, str(args.fixed_fraction), str(num_nodes), args.type_sim, args.network_type) ):
        os.makedirs(os.path.join(results_path, args.game_strategy, str(args.fixed_fraction), str(num_nodes), args.type_sim, args.network_type))
        
    if not os.path.isdir( os.path.join(results_path, args.game_strategy, str(args.fixed_fraction), str(num_nodes), args.type_sim, args.network_type, 'checkpoints') ):
        os.makedirs(os.path.join(results_path, args.game_strategy, str(args.fixed_fraction), str(num_nodes), args.type_sim, args.network_type, 'checkpoints'))

    path_to_save_checkpoints = os.path.join(results_path,
                                            args.game_strategy,
                                            str(args.fixed_fraction), 
                                            str(num_nodes), 
                                            args.type_sim, 
                                            args.network_type, 
                                            'checkpoints', 
    ...
    df_response = models_game_elasticity.run_model(models_game_elasticity.sis_replicator, 
                                                G, 
                                                params=model_params, 
                                                n_iters=n_iters, 
                                                ...
                                                num_checkpoints=0, 
                                                local=local, 
                                                gameFixedFraction=args.fixed_fraction,
                                                gameFixedStrategy=args.game_strategy,
